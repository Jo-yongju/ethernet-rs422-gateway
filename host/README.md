...
g++ host\tcp_recovery_client.cpp crc16.o protocol.o stream_parser.o -Icommon -std=c++17 -Wall -Wextra -Wpedantic -lws2_32 -o tcp_recovery_client.exe
...
```powershell
.\tcp_client.exe --ip 192.168.77.2 --port 5000 --count 1000 --timeout-ms 1000
.\tcp_recovery_client.exe
.\udp_client.exe --port 5002 --expected 1000 --initial-timeout-ms 10000 --idle-timeout-ms 1500
.\fault_client.exe --ip 192.168.77.2 --port 5000 --test bad-crc
.\fault_client.exe --test unknown-msg --dry-run
```

`tcp_recovery_client` is a separate cable-recovery test tool. It targets
`192.168.77.2:5000`, retries bounded non-blocking connections every 50 ms,
sends a Protocol V1 PING every 100 ms, and reconnects after a 200 ms PONG
timeout. It does not replace `tcp_client`.
